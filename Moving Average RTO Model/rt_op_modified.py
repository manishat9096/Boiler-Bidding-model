...
class Realtime_Optimisation():
    ...
    def load_csv(self):
        ...
        rolling_date = self.date - timedelta(hours= 24)
        df = df[(df['SecondUTC'] >= rolling_date) & (df['SecondUTC'] <= self.end)].reset_index(drop=True)
        rolling = True; shift = False
        if rolling == True:
            # implement rolling mean of afrr prices
            rolling_window = '6h' # rolling mean calculated for this window
            shift_by = 30 #minutes
            if shift == True:
                shifted = df.set_index('SecondUTC')
                shifted.index = shifted.index + pd.Timedelta(minutes=shift_by)
                rolling_afrr = shifted.rolling(rolling_window, closed='left').mean()
                rolling_afrr = rolling_afrr.reset_index() 
            else:
                rolling_afrr = df.rolling(rolling_window, closed='left', on= 'SecondUTC').mean()
            df = df.rename(columns={"aFRR_UpActivatedPriceEUR": "aFRR_UpActivatedPriceEUR_actual", "aFRR_DownActivatedPriceEUR": "aFRR_DownActivatedPriceEUR_actual"}) # rename actual price columns
            df = df.merge(rolling_afrr[['SecondUTC','aFRR_UpActivatedPriceEUR','aFRR_DownActivatedPriceEUR']], on='SecondUTC')
            df = df[df['SecondUTC'] >= self.date].reset_index(drop=True)
        ...
        self.boiler_schedule = self.boiler_schedule[['UTC','Heat load forecast [MW]','L11 EK plan [MW]', 'Capacity Bid']]
    ...
    def optmize(self, Einit, W_factor, plot_flag = False):
        ...
        if self.md.status == GRB.OPTIMAL:
            ...
            deviation = sum((power_boiler[t] - np.ceil(self.boiler_schedule['L11 EK plan [MW]'][t]))**2 for t in range(self.horizon))
            ...
            print(f'calculate objective: {self.system_data['W'] * deviation - revenue}')
            flattened_data = {
