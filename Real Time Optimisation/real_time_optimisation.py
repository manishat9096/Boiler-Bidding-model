...
def steady_state_multiple_MTU_OP(price_data, columns, boiler_schedule, system_data, horizon):
    ...
    deltaT = 15/60  #4s interval in a hour
    deltaK = 4/3600
    M = 10e6
    ...
    model.addConstr(eff_SOE * Einit/Emax - (deltaT*P_up[0])/Emax >= 0.1, name = f'Available_Mwh_UP_{0}')
    model.addConstr(eff_SOE * Einit/Emax + (deltaT*P_down[0])/Emax <= 0.9, name = f'Available_Mwh_Down_{0}')
    for t in T[1:]:
        model.addConstr(eff_SOE * SoE[t-1, len(seconds_MTU)-1] - (deltaT*P_up[t])/Emax >= 0.1, name = f'Available_Mwh_UP_{t}')
        model.addConstr(eff_SOE * SoE[t-1, len(seconds_MTU)-1] + (deltaT*P_down[t])/Emax <= 0.9, name = f'Available_Mwh_Down_{t}')

    for t in T:
        for k in range(len(seconds_MTU)):
            model.addConstr(E_up[t,k] <= eff_P2H * P_up[t] * deltaK * u[t], name = f'E_UP_1_at_{t}MTU_{seconds_MTU[k]}s')
            model.addConstr(E_up[t,k] >= eff_P2H * P_up[t] * deltaK * u[t] - M * (1 - beta[t,k]), name = f'E_UP_2_at_{t}MTU_{seconds_MTU[k]}s')
            model.addConstr(E_up[t,k] <= M * beta[t,k], name = f'E_UP_3_at_{t}MTU_{seconds_MTU[k]}s')

            model.addConstr(E_down[t,k] <= eff_P2H * P_down[t] * deltaK * (1 - u[t]), name = f'E_DOWN_1_at_{t}MTU_{seconds_MTU[k]}s')
            model.addConstr(E_down[t,k] >= eff_P2H * P_down[t] * deltaK * (1 - u[t]) - M * (1 - beta[t,k]), name = f'E_DOWN_2_at_{t}MTU_{seconds_MTU[k]}s')
    ...
    model.addConstr(SoE[0,0] == eff_SOE_4s * Einit/Emax + (deltaK * P_boiler[0] + E_down[0,0] - E_up[0,0])/Emax  - (deltaK * demand[0])/Emax, name = f'System_state_after_{0}MTU_{seconds_MTU[0]}s')

    for k in range(len(seconds_MTU))[1:]:
        model.addConstr(SoE[0,k] == eff_SOE_4s * SoE[0,k-1] + (deltaK * P_boiler[0] + E_down[0,k] - E_up[0,k])/Emax  - (deltaK * demand[0])/Emax, name = f'System_state_after_{0}MTU_{seconds_MTU[k]}s')

    for t in T[1:]:
        model.addConstr(SoE[t,0] == eff_SOE_4s * SoE[t-1, len(seconds_MTU)-1] + (deltaK * P_boiler[t] + E_down[t,0] - E_up[t,0])/Emax  - (deltaK * demand[t])/Emax, name = f'System_state_after_{t}MTU_{seconds_MTU[0]}s')
        for k in range(len(seconds_MTU))[1:]:
            model.addConstr(SoE[t,k] == eff_SOE_4s * SoE[t,k-1] + (deltaK * P_boiler[t] + E_down[t,k] - E_up[t,k])/Emax  - (deltaK * demand[t])/Emax, name = f'System_state_after_{t}MTU_{seconds_MTU[k]}s')
...
if __name__ == '__main__':
    horizon = 96 # 15 min blocks over a day
    ...
    # linheat_df.loc[30:40, 'L11 EK plan [MW]'] = 12
