...
class RecursiveRTO():
    def __init__(self, start_date, end_date, print_flag = False, plot_flag = False):
        ...
        self.schedulefile = 'LinHeat_schedule_changed_for_paper.csv'
        ...
        self.imbalancefile = 'imbalanceprices_2025-02-25_to_2025-04-01.csv'
        ...
        self.output = []
        self.Einit = 39 # Start the simulation with 90% SoE
    ...
    def ARX_model(self, results):
        ...
        if ARX_simulator == True:
            ...
            self.Einit = results.loc[results['SecondUTC'] == self.current + timedelta(hours=self.recurssive_period), 'System SoE (%)'].iloc[0] * 60 # Emax
    ...
    def save_results(self):
        ...
        final_results['MTU Revenue Row'] = (
            final_results['Energy Up Cleared (MWh)'] * final_results['aFRR_UpActivatedPriceEUR'] +
            final_results['Energy Down Cleared (MWh)'] * -final_results['aFRR_DownActivatedPriceEUR'])
    ...
    def run_RTO_recursively(self, w = 0, horizon = 96):
        while self.current <= self.end:
            ...
            RTO = Realtime_Optimisation(day, self.afrrfile, self.spotpricefile, self.schedulefile, self.imbalancefile,horizon= horizon)
            # if day == self.start:
