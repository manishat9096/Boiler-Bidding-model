...
def run_RTO(start_date, end_date, w):
    ...
    schedulefile = 'LinHeat_schedule_changed_for_paper.csv'
    imbalancefile = 'imbalanceprices_2025-02-25_to_2025-04-01.csv'
    ...
    while current <= end:
        ...
        RTO = Realtime_Optimisation(day, afrrfile, spotpricefile, schedulefile, imbalancefile, horizon= 96)
        if day == start_date:
            einit = RTO.get_Einit(start_date)
            model, results = RTO.optmize(Einit = einit, W_factor = w)
        else:
            einit = RTO.get_Einit(day)
            model, results = RTO.optmize(Einit = einit, W_factor = w)
        if model.status != 2:
        ...
        day_iterator.update(1)
        row = results[['Deviation','Total Revenue','UTC']].loc[1]
        row['W_factor'] = w
        output.append(row)
    day_iterator.close()
    return output
...
def plot_results(results):
    ...
    fig.add_trace(go.Scatter(x=results['SecondUTC'], y=results['Energy Up Cleared (MWh)'], mode='lines+markers', name='Energy Up Cleared (MWh)'), row=2, col=1)
    fig.add_trace(go.Scatter(x=results['SecondUTC'], y=results['Energy Down Cleared (MWh)'], mode='lines+markers', name='Energy Down Cleared (MWh)'), row=2, col=1)
...
if __name__ == '__main__': 
    ...
    end_date = '2025-03-07 23:45:00' #YYYY-MM-DD
    ...
    if select_case == 'base':
        ...
        results.to_csv(folder_path + f'recursive_MARCH_2_to_16_w1000_4h_actual_prices_4.csv', index=False)

    if select_case == 'W_factor':
        # to run the model for different W_factors
        W_factors = np.arange(0, 201, 10)
        output = []
        ...
        W_revenue.to_csv('deviation_revenue_vs_w_factor_with_penalty_value.csv', index = False)

    if select_case == 'prediction_horizon':
        w = 0
        # to run the model for different horizon
        horizon_list = np.sort(np.append(np.arange(8, 97, 8), [4]))
        # horizon_list = np.arange(24, 49, 24)
        output = []
        for h in tqdm(horizon_list):
            print(f'RTO for horizon: {h} from {start_date} to {end_date}')
            start_timer = time.time() 
            RTO = RecursiveRTO(start_date, end_date, print_flag= True, plot_flag= False)
            df = RTO.run_RTO_recursively(w = w, horizon= h)
            computation_time = time.time() - start_timer
            df['direction'] = np.where((df['activation'] == 1) & (df['Bid Up (MW)'] >= 0), 1, 
                                np.where((df['activation'] == 0) & (df['Bid Down (MW)'] >= 0), -1, 
                                np.where((df['Bid Up (MW)'] == 0) & (df['Bid Down (MW)'] == 0), 0, np.nan)))
        ...
        # horizon_revenue.to_csv(folder_path + f'prediction_horizon_07_March_with_W{w}.csv', index=False)
